# DRAFT - Geocoding Addresses with locator.stanford.edu and OpenRefine
...
> **Concept note:** Geocoding is the process of turning a place description, such as a street address, into map coordinates. The result is usually a pair of longitude and latitude values that can be used to create point geometry in GIS software.
...
- why an address table must have consistent address fields before geocoding
- how a URL can carry query parameters to a web service
- why API responses are often returned as JSON
- how OpenRefine can repeat a web request across many table rows
...
Web APIs allow you to submit a URL just like when you type a URL into your browser address bar. The important difference is that the URL we are submitting has a bit of data, such as an `address` or `coordinate pair`, that we want to know something about. Rather than giving us back an `HTML` file that instructs our browser to retrieve images, text, and other objects and arrange them into a webpage, the web API we will use gives us back information about the `address` data we submit.

> **Concept note:** A REST API is designed around URLs. The base part of the URL identifies the service, and the query parameters after the `?` provide the specific inputs and options for a request.
...
> **Concept note:** Query parameters are the `key=value` pieces after the `?` in a URL. In this example, `City=SAN+JOSE` tells the service which city value to use, and `maxLocations=1` tells it to return only the best candidate.
...
> **Concept note:** JSON stores data in nested structures. This is powerful because one API response can include coordinates, match scores, full addresses, and metadata, but it also means you need to know the path to the value you want.
